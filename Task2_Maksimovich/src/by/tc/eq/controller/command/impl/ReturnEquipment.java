...
public class ReturnEquipment implements Command {
	...
	private static final Logger logger = LogManager.getLogger(ReturnEquipment.class.getName());
	...
	public String execute(String request) {
		...
		if (params.length >= EXPECTED_MIN_QUANTITY_OF_PARAMETERS
			...
			// размер_массива - количество возвращаемых товаров
			...
			try {
				...
				shopService.returnEquipment(id_user, ids_equipment);
				response = "The equipment was reterned";

			} catch (ServiceException e) {
				response = "Error during equipment returning!";
				logger.error(response);
				logger.log(Level.ERROR, e);
			}
