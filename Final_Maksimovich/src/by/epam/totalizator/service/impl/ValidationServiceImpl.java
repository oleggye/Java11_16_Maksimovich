...
public class ValidationServiceImpl implements ValidationService {
	...
	public void validateUser(User user) throws ServiceValidationException {
		...
		if (user != null) {
			...
			if (validator.isFirstNameInvalid(user.getFirstName())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), FIRST_NAME_ERROR_KEY);
				errorSet.add(message);
			}
			if (validator.isLastNameInvalid(user.getFirstName())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), LAST_NAME_ERROR_KEY);
				errorSet.add(message);
			}

			if (validator.isEmailInvalid(user.getEmail())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), EMAIL_ERROR_KEY);
				errorSet.add(message);
			}

			if (validator.isDateOfBirthInvalid(user.getDateOfBirth())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), SECRET_QUESTION_ERROR_KEY);
				errorSet.add(message);
			}

			if (validator.isCountryInvalid(user.getCountry())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), COUNTRY_ERROR_KEY);
				errorSet.add(message);
			}

			if (validator.isPhoneInvalid(user.getPhone())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), PHONE_ERROR_KEY);
				errorSet.add(message);
			}

			if (validator.isCurrencyInvalid(user.getCurrency())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), CURRENCY_ERROR_KEY);
				errorSet.add(message);
			}

			if (validator.isDateOfBirthInvalid(user.getDateOfBirth())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), BIRTH_DATE_ERROR_KEY);
				errorSet.add(message);
			}

			if (validator.isPasswordInvalid(user.getPassword())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(), PASSWORD_ERROR_KEY);
				errorSet.add(message);
			}
			...
			if (validator.isSecretAnswerInvalid(user.getSecredQuestionAnswer())) {
				message = LocalizationBundle.getInstance().getProperty(user.getLocale(),
						SECRET_QUESTION_ANSWER_ERROR_KEY);
				errorSet.add(message);
			}
	...
	public void validateLogin(String login) throws ServiceValidationException {
		if (validator.isEmailInvalid(login)) {
			String message = LocalizationBundle.getInstance().getProperty(Locale.getDefault(), EMAIL_ERROR_KEY);
	...
	public void validatePassword(String password) throws ServiceValidationException {
		if (validator.isPasswordInvalid(password)) {
			String message = LocalizationBundle.getInstance().getProperty(Locale.getDefault(), PASSWORD_ERROR_KEY);
			throw new ServiceValidationException(message);
		}
	}
	...
	public void validateStartTime(Date startTime) throws ServiceValidationException {
		...
		if (validator.isDateOfBirthInvalid(startTime)) {
	...
	public void validateHomeRate(BigDecimal homeRate) throws ServiceValidationException {
		...
		if (validator.isBetValueInvalid(homeRate)) {
	...
	public void validateDrawRate(BigDecimal drawRate) throws ServiceValidationException {
		if (validator.isBetValueInvalid(drawRate)) {
	...
	public void validateAwayRate(BigDecimal awayRate) throws ServiceValidationException {
		if (validator.isBetValueInvalid(awayRate)) {
	...
	public void validateBetting(Betting betting) throws ServiceValidationException {
		...
		if (validator.isBetValueInvalid(betting.getBetRate())) {
			throw new ServiceValidationException("Wrong param: betRate=" + betting.getBetRate());
		}

		if (validator.isBetValueInvalid(betting.getBetSize())) {
	...
	private static class Validator {
		...
		private static final String COUNTRY_REGEXP = "^[A-Za-z]{3,14}";
		...
		boolean isCountryInvalid(Country country) {

			if (country == null || isIdInvalid(country.getId())) {
				return true;
			}
			Pattern pattern = Pattern.compile(COUNTRY_REGEXP);
		...
		boolean isDateOfBirthInvalid(Date date) {
			...
			if (fullYearDifference == LOWER_YEAR_BOUND) {
				...
				if (nowDateMonthNumber > dateMonthNumber
						|| (nowDateMonthNumber == dateMonthNumber && nowDateDayNumber >= dateDayNumber)) {
					return false;
				}
			}
		...
		boolean isBetValueInvalid(BigDecimal rate) {
			BigDecimal standard = new BigDecimal(0);

			if (rate == null || rate.compareTo(standard) != 1) {
				return true;
			}
			return false;
		}
