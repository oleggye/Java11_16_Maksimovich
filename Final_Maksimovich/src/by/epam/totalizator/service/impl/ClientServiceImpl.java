...
public class ClientServiceImpl implements ClientService {
	...
	private static final String PARAM_NAME_LOCALE = "local";
	...
	public void signIn(SessionRequestContent requestContent) throws ServiceException, ServiceValidationException {
		...
		try {
			...
			requestContent.putSessionAttribute(PARAM_NAME_USERID, user.getId());
			requestContent.putSessionAttribute(PARAM_NAME_LOGIN, user.getEmail());
			requestContent.putSessionAttribute(PARAM_NAME_USERTYPE, user.getUserType());
			requestContent.putSessionAttribute(PARAM_NAME_LOCALE, user.getLocale().toString());
	...
	public void registration(SessionRequestContent requestContent) throws ServiceException, ServiceValidationException {
		...
		User user = new UserBuilder().buildFirstName(requestContent.getRequestParam(PARAM_NAME_FIRST_NAME))
				...
				.buildCountry(requestContent.getRequestParam(PARAM_NAME_COUNTRY))
				.buildPhone(new Phone(requestContent.getRequestParam(PARAM_NAME_PHONE_CODE),
						requestContent.getRequestParam(PARAM_NAME_PHONE_NUMBER)))
				...
				.buildSecredQuestionAnswer(requestContent.getRequestParam(PARAM_SECRET_ANSWER)).build();
