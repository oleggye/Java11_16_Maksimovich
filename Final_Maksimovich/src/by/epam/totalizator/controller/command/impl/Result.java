...
public class Result implements Command {
	...
	private static final String RESULT_PAGE_URL = "path.page.result";
	...
	private static final String ATTRIBUTE_COMMAND_VALUE = "result";
	...
	public String execute(HttpServletRequest request) {
		...
		try {
			List<Competition> competitions = factory.getCompetitionService().obtainCompetitionResultList(pageNumber,
					RECORDS_PER_PAGE);
			request.setAttribute(ATTRIBUTE_NAME_COMPETITIONS, competitions);
			page = ConfigurationManager.getProperty(RESULT_PAGE_URL);
			int availableCount = factory.getCompetitionService().obtainCompetitionResultCount();
