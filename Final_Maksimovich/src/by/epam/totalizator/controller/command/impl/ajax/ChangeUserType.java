 ...
 */
public class ChangeUserType implements ICommand {

	private static final Logger LOGGER = LogManager.getLogger(ChangeUserBalance.class.getName());

	private static final String RESPONSE_CONTENT_TYPE = "text/plain";

	private static final String SUCCESS_MESSAGE_KEY = "local.ajax.change_user_type.success";
	...
	/**
	 * Performs a service level call to change {@link UserType} of the user.
	 * 
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		try {
			...
			factory.getClientService().changeUserType(idUser, userType);
	...
	/**
	 * Method gets new user type string from the user's request and get instance
	 * of {@link UserType}
	 */
	private UserType getUserType(HttpServletRequest request) {
		String userTypeParam = request.getParameter(ParamNameStore.PARAM_NAME_USERTYPE);
		return UserType.getTypeByShortName(userTypeParam);
	}
