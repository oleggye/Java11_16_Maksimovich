...
public class Home implements Command {
	...
	private static final String COMMAND_NAME_PAGE_NUMBER = "command";
	...
	public String execute(HttpServletRequest request) {
		...
		try {
			List<Competition> competitions = factory.getCompetitionService().getAvailableCompetitions(pageNumber,
					RECORDS_PER_PAGE);
			request.setAttribute(ATTRIBUTE_NAME_COMPETITIONS, competitions);
			page = ConfigurationManager.getProperty(HOME_PAGE_URL);
			int availableCount = factory.getCompetitionService().getAvailableCompetitionsCount();

			double result = (double) availableCount / RECORDS_PER_PAGE;
			// TODO: предполагается, что long смноговато
