...
public class TeamDAOImpl implements TeamDAO {
	...
	public List<Team> obtainTeamListByIdSport(int idSport, Locale locale) throws DAOException {
		...
		SQLName sqlName = locale.equals(Locale.ENGLISH) ? SQLName.GET_TEAM_LIST_BY_ID_SPORT_EN
			...
			try (PreparedStatement prepStatement = connection
				...
				try (ResultSet resultSet = prepStatement.executeQuery()) {
					...
					while (resultSet.next()) {
						Club club = new ClubBuilder().buildId(resultSet.getInt(1)).buildName(resultSet.getString(2))
								.build();
						Country country = new CountryBuilder().buildId(resultSet.getInt(3))
								.buildName(resultSet.getString(4)).build();
						Team team = new TeamBuilder().buildClub(club).buildCountry(country).build();
						teamList.add(team);
					}
				}
			}
