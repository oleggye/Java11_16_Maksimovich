...
function checkCityField(cityField) {
	var backgroundColorName = "inherit";
	var citySpanName = 'city-span';
	var errorMessage = '';
	var messageColor = 'inherit';

	if (cityField.checkValidity() == false) {
		errorMessage = cityField.title;
		messageColor = 'red';
		backgroundColorName = "rgba(255,0,0,0.3)";
	}
	cityField.style.backgroundColor = backgroundColorName;
	setSpanText(citySpanName, errorMessage, messageColor);
}
...
function validateDateField(dateField) {
	...
	if (!isNaN(date)) {
		...
		errorMessage = 'Your age have to be between ' + LOWER_YEAR_BOUND
				+ ' and ' + UPPER_YEAR_BOUND;
	} else {
