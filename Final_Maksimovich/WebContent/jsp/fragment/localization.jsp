...
<div class="droplocdown">
	...
	<c:choose>
		...
		<c:when test="${local eq 'EN' }">
			...
			<div class="droplocdown-content">
				<a href="#"><fmt:message bundle="${localBundle}"
						key="local.locbutton.name.en" /></a> <a
					href="controller?command=home&local=RU"><fmt:message
						bundle="${localBundle}" key="local.locbutton.name.ru" /></a>
		...
		<c:otherwise>
			...
			<div class="droplocdown-content">
				<a href="controller?command=home&local=EN"> <fmt:message
						bundle="${localBundle}" key="local.locbutton.name.en" /></a> <a
					href="#"><fmt:message bundle="${localBundle}"
						key="local.locbutton.name.ru" /></a>
			</div>
		</c:otherwise>
	</c:choose>
</div>